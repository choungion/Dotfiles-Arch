...
try:
    ...
    print("----------------------------------------------------------------------")
    print("FATAL ERROR: Could not import required Python libraries.")
    print(f"Missing module: {e.name}")
    print("Please install the necessary packages. Examples:")
    print("  - Debian/Ubuntu: sudo apt install python3-dbus python3-gi")
    print("  - Arch/Manjaro:  sudo pacman -S python-gobject python-dbus")
    print("----------------------------------------------------------------------")
    sys.exit(1)

# Keep the remaining imports from your original file:
import datetime
import typing
import json
import subprocess
...
try:
    # Use the full flag name if available (modern dbus-python)
    DBUS_REPLACE_EXISTING = dbus.BUS_NAME_FLAG_REPLACE_EXISTING
except AttributeError:
    # Fallback to the numeric value if the attribute is missing (older dbus-python)
    DBUS_REPLACE_EXISTING = 2
...
class NotificationDaemon(dbus.service.Object):
    def __init__(self):
        ...
        # 2. **MANUALLY REQUEST THE NAME HERE**, passing the flag.
        #    This is what successfully registers your daemon.
        bus.request_name("org.freedesktop.Notifications", DBUS_REPLACE_EXISTING)
        
        # 3. Initialize the bus name object WITHOUT the flags argument.
        bus_name = dbus.service.BusName("org.freedesktop.Notifications", bus)
        
        # 4. Initialize the DBus service object.
